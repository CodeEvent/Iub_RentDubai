    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING(
                ...
                "access.\n"
                f"  - {LAWYER_GROUP_NAME} membership and its restricted-reason "
                "object grants only apply going forward (Workflow triggers "
                "fire on DOCUMENT_ADDED, not retroactively) -- notices "
                "created before both commands were run need their "
                "permissions set by hand (Documents > select > Edit "
                "permissions) if they should be restricted.\n"
